...
class caffe_objecet_direction():
    ...
    def filter_objects(inference_result, input_image_width, input_image_height):
        ...
        caffe_objecet_direction.boxes_to_pixel_units(all_boxes, input_image_width, input_image_height, grid_size)
        ...
        duplicate_box_mask = caffe_objecet_direction.get_duplicate_box_mask(boxes_above_threshold)
    ...
    def get_duplicate_box_mask(box_list):
        ...
        for i in range(len(box_list)):
            ...
            for j in range(i + 1, len(box_list)):
                if caffe_objecet_direction.get_intersection_over_union(box_list[i], box_list[j]) > max_iou:
                    box_mask[j] = 0.0
    ...
    def main():
        ...
        input_image_file = os.path.join(os.path.dirname(__file__), input_image_file_)
        ...
        filtered_objs = caffe_objecet_direction.filter_objects(output.astype(np.float32), input_image.shape[1],
                                                               input_image.shape[
                                                                   0])  # fc27 instead of fc12 for yolo_small
